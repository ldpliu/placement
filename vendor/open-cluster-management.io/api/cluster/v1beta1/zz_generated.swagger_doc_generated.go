...
var map_ManagedClusterSelector = map[string]string{
	"selectorType":  "\"\" means to use the current mechanism of matching label <cluster.open-cluster-management.io/clusterset:<ManagedClusterSet Name>. \"LabelSelector\" means to use the LabelSelector to select target managedClusters \"ClusterLabel\" means to use a particular cluster label. It is guaranteed that clustersets with same label key are exclusive with each others",
	"clusterLabel":  "ClusterLabel defines one label which clusterset could use to select target managedClusters. In this way, we will: 1. Guarantee clustersets with same label key are exclusive 2. Enable additional permission check when cluster joining/leaving a clusterset (the label key should start with the reserved prefix \"cluster.open-cluster-management.io/\" and \"info.open-cluster-management.io/\");",
...
var map_ManagedClusterSetSpec = map[string]string{
	"clusterSelector": "Selector represents a selector of ManagedClusters by labels and names.",
}
